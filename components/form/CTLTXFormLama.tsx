    ...
    const handleMergeClips = async () => {
        ...
        if (ffmpegLoadState !== 'loaded') {
            try {
                ...
                addLog('[EA-LTX] Gagal memuat FFmpeg. Tidak dapat menggabungkan klip.', 'error');
                return;
            }
        }
        ...
        if (scenesToMerge.length < 1) {
            addLog('[EA-LTX] Butuh setidaknya 1 klip video yang sudah dibuat untuk digabungkan.', 'warning');
            return;
        }
    
        setIsProcessing(true);
        addLog(`[EA-LTX] Memulai penggabungan untuk ${scenesToMerge.length} klip... Ini mungkin memakan waktu beberapa menit.`, 'status');
        ...
        try {
            ...
            for (let i = 0; i < scenesToMerge.length; i++) {
                ...
                addLog(`[EA-LTX] Membaca klip ${i + 1} ke memori...`, 'info');
            ...
            addLog('[EA-LTX] Menggabungkan klip dengan FFmpeg... Ini adalah langkah terlama.', 'status');
            ...
            addLog('[EA-LTX] Penggabungan selesai. Membaca file output...', 'info');
            const data = await ffmpeg!.readFile(outputFilename);
            
            triggerDownload(URL.createObjectURL(new Blob([data], { type: 'video/mp4' })), outputFilename);
            addLog(`[EA-LTX] Unduhan dimulai untuk "${outputFilename}".`, 'info');
