...
const CTLTXForm: React.FC<CTLTXFormProps> = ({
    ...
    const loadFFmpeg = useCallback(async () => {
        ...
        addLog('[EA-LTX] Memuat FFmpeg (sesuai permintaan)...', 'info');
        ...
        try {
            ...
            addLog('[EA-LTX] FFmpeg berhasil dimuat.', 'info');
            return newFFmpeg;
        } catch (err: any) {
            setFfmpegLoadState('error');
            addLog(`[EA-LTX] Gagal memuat FFmpeg: ${err.message}.`, 'error');
            throw err;
        }
    ...
    const handleGenerateScript = async () => {
        ...
        addLog(`[EA-LTX] Membuat naskah untuk: "${ltxProject.title}"...`, 'status');
    
        try {
            // STEP 1: Generate Outline
            const [outlineResponse]: [GenerateContentResponse, any] = await executeApiCallWithKeyRotation(async (ai) => {
                addLog('[EA-LTX] Langkah 1/2: AI Director sedang membuat kerangka alur cerita...', 'status');
            ...
            addLog(`[EA-LTX] Kerangka alur cerita dibuat. Langkah 2/2: Menulis detail untuk ${outline.length} adegan...`, 'status');
    
            // STEP 2: Generate details for each scene
            const newScenes: LtxScene[] = [];
            for (let i = 0; i < outline.length; i++) {
                const sceneOutline = outline[i];
                addLog(`[EA-LTX] Menulis detail untuk Adegan ${i + 1}/${outline.length}...`, 'status');
            ...
            addLog(`[EA-LTX] Naskah berhasil dibuat dengan ${newScenes.length} adegan.`, 'info');
    
        } catch (error: any) {
            addLog(`[EA-LTX] Error membuat naskah: ${getFriendlyApiErrorMessage(error)}`, 'error');
        } finally {
    ...
    const handleGenerateVisualsForScene = useCallback(async (sceneId: string, type: 'image' | 'video'): Promise<string | undefined> => {
        ...
        if (!scene || !project) {
            addLog(`[EA-LTX] Adegan ${sceneId} tidak ditemukan.`, 'error');
            return;
        }

        if (type === 'video' && !scene.imageBase64) {
            addLog(`[EA-LTX] Buat gambar terlebih dahulu untuk Adegan ${scene.sceneNumber}.`, 'warning');
            return;
        }
        ...
        addLog(`[EA-LTX] Adegan ${scene.sceneNumber}: Membuat ${type}...`, 'status');
        ...
        try {
            ...
            if (type === 'image') {
                ...
                logUsage('images');
                addToMediaLibrary({ type: 'image', previewUrl: imageUrl, prompt: scene.imagePrompt, model: 'imagen-4.0-generate-001', sourceComponent: 'EA-LTX', base64: imageBytes, mimeType: 'image/png' });
                return imageBytes;
            } else { // video
                ...
                reader.onloadend = () => addToMediaLibrary({ type: 'video', previewUrl: reader.result as string, prompt: scene!.videoPrompt, model: project!.videoModel, sourceComponent: 'EA-LTX', base64: (reader.result as string).split(',')[1], mimeType: 'video/mp4' });
            }
        } catch (err: any) {
            const friendlyError = getFriendlyApiErrorMessage(err);
            addLog(`[EA-LTX] Gagal pada Adegan ${scene.sceneNumber} (${type}): ${friendlyError}`, 'error');
    ...
    const handleGenerateAllVisuals = async () => {
        ...
        if (scenesToProcess.length === 0) {
            addLog("[EA-LTX] Tidak ada adegan untuk diproses. Buat naskah terlebih dahulu.", 'info');
            return;
        }
        setIsGeneratingAll(true);
        setStatus('Running');
        runningRef.current = true;
        addLog(`[EA-LTX] Memulai pembuatan visual untuk ${scenesToProcess.length} adegan...`, 'status');
    
        for (const scene of scenesToProcess) {
            if (!runningRef.current) {
                addLog("[EA-LTX] Pembuatan dihentikan oleh pengguna.", "warning");
                break;
            }
        ...
        addLog('[EA-LTX] Proses pembuatan semua visual selesai.', 'info');
    };

    const handleGenerateAllVideos = async () => {
        const scenesToProcess = ltxProject.scenes.filter(s => s.imageBase64 && !s.videoUrl && s.status !== 'generating-video');
        if (scenesToProcess.length === 0) {
            addLog("[EA-LTX] Tidak ada adegan dengan gambar yang siap untuk pembuatan video.", 'warning');
            return;
        }
        setIsGeneratingAll(true);
        setStatus('Running');
        runningRef.current = true;
        addLog(`[EA-LTX] Memulai pembuatan video untuk ${scenesToProcess.length} adegan...`, 'status');
    
        for (const scene of scenesToProcess) {
            if (!runningRef.current) {
                addLog("[EA-LTX] Pembuatan video dihentikan oleh pengguna.", "warning");
                break;
            }
            await handleGenerateVisualsForScene(scene.id, 'video');
        }
        ...
        setStatus(runningRef.current ? 'Done' : 'Idle');
        runningRef.current = false;
        addLog('[EA-LTX] Semua tugas pembuatan video telah selesai.', 'info');
    };
    ...
    const handleReset = () => {
        if (window.confirm("Apakah Anda yakin ingin memulai proyek baru? Semua perubahan yang belum disimpan akan hilang.")) {
            ...
            addLog("[EA-LTX] Proyek baru dimulai.", 'info');
        }
    };
    ...
    const handleMergeClips = async () => {
        ...
        if (scenesToMerge.length < 1) {
            addLog('[EA-LTX] Butuh setidaknya 1 klip video & audio yang sudah dibuat untuk digabungkan.', 'warning');
            return;
        }
    
        setIsProcessing(true);
        addLog(`[EA-LTX] Memulai penggabungan untuk ${scenesToMerge.length} klip...`, 'status');
        ...
        try {
            ...
            addLog(`[EA-LTX] Unduhan film dimulai.`, 'info');
    
        } catch (error: any) {
            addLog(`[EA-LTX] Error saat penggabungan: ${error.message}`, 'error');
        } finally {
    ...
    const handleDownloadZip = async () => {
        ...
        if (scenesToZip.length === 0) { addLog("[EA-LTX] Tidak ada adegan yang selesai untuk di-zip.", 'warning'); return; }
        setIsProcessing(true);
        addLog(`[EA-LTX] Menyiapkan file ZIP...`, 'status');
        try {
            const zip = new JSZip();
    ...
    const handleImportCharacters = () => {
        ...
        addLog(`[EA-LTX] Mengimpor ${newChars.length} karakter ke proyek.`, 'info');
    ...
    const handlePlayAll = () => {
        if (scenesWithVideo.length === 0) {
            addLog("[EA-LTX] Tidak ada klip yang selesai untuk diputar.", 'warning');
            return;
        }
        addLog('[EA-LTX] Memulai pemutaran semua klip...', 'info');
    ...
    const handlePlaybackEnded = useCallback(() => {
        setPlayAllState(prev => {
            ...
            if (nextIndex >= scenesWithVideo.length) {
                addLog('[EA-LTX] Pemutaran selesai.', 'info');
    ...
    useEffect(() => {
        ...
        if (sceneToPlay) {
            ...
            player.play().catch(e => {
                addLog(`[EA-LTX] Gagal memutar video secara otomatis: ${e.message}`, 'warning');
                handleStopPlayAll();
            });
        } else {
            handleStopPlayAll();
    ...
    const handleImageUploadForScene = (sceneId: string, imageData: { name: string; base64: string; mimeType: string; dataUrl: string; }) => {
        ...
        addLog(`[EA-LTX] Gambar "${imageData.name}" diunggah untuk adegan.`, 'info');
    };
    ...
    const handleAddManualScene = () => {
        ...
        addLog('[EA-LTX] Adegan manual baru ditambahkan.', 'info');
    };
