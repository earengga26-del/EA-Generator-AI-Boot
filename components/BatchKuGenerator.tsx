            ...
            <fieldset disabled={isLoading} className="space-y-6 flex-grow">
              <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <SelectInput<AspectRatio> label="Rasio Aspek" value={aspectRatio} onChange={setAspectRatio} options={[{ value: '16:9', label: '16:9' }, { value: '9:16', label: '9:16' }]} />
                </div>
              </div>
              ...
              {outputs.map((output, index) => (
                <div key={output.id} className="space-y-2 bg-slate-800 p-3 rounded-lg">
                  ...
                  {output.status === 'done' && output.url ? (
                    <><VideoPlayer videoUrl={output.url} /><Button variant="secondary" onClick={() => handleDownload(output.url!, index)} className="w-full mt-2"><DownloadIcon className="w-5 h-5" />{t('downloadVideoButton')}</Button></>
                  ) : output.status === 'generating' ? (
                    <div className="aspect-video bg-black rounded-md flex flex-col items-center justify-center text-gray-400"><Spinner className="h-8 w-8"/><p className="mt-2 text-sm">{t('generatingSceneShort')}</p></div>
                  ) : (
                    <div className="aspect-video bg-black rounded-md flex items-center justify-center text-gray-600 gap-2"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><p className="text-sm font-semibold">{t('scenePending')}</p></div>
                  )}
                </div>
              ))}
            </div>
